...
import { bouwLeenSnapshot, trekUitleningIn, definitiefOverzetten } from './teams-spelers.js?v=20260924b';
...
export async function openBouwHub(clubId, bouw, isHerbezoek){
  ...
  if (!isHerbezoek && !window.matchMedia('(min-width:1100px) and (min-height:520px)').matches){
    try { const m = await import('./mobiel-bouw.js?v=20260924b'); await m.openMobielBouw(clubId, bouw); return; }
...
function renderTeamsScherm(){
  ...
  inhoud.querySelectorAll('[data-bh-open-team]').forEach(b => {
    b.onclick = async () => {
      ...
      const m = await import('./teams.js?v=20260924b');
