...
function _navTerug(scherm){
  import('./tracker.js?v=20260828d').then(m => m.telNav?.(scherm, 'terug')).catch(()=>{});
}
...
function stapTerug(){
  ...
  if (lightboxOpen()){
    import('./training-weergave.js?v=20260828d').then(m => m.sluitLightbox());
    return true;
  }
  if (pdfViewerOpen()){
    import('./pdf-viewer.js?v=20260828d').then(m => m.sluitPdfViewer());
    return true;
  }
