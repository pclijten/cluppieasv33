...
function _navTerug(scherm){
  import('./tracker.js?v=20260902b').then(m => m.telNav?.(scherm, 'terug')).catch(()=>{});
}

/* Eén terug-stap volgens prioriteit. true = afgehandeld (app blijft open). */
function stapTerug(){
  if (tactiekbordOpen()){
    import('./tactiekbord.js?v=20260902b').then(m => m.sluitTactiekbord());
    return true;
  }
  if (leerpleinOpen()){
    import('./leerplein.js?v=20260902b').then(m => m.sluitLeerplein());
    return true;
  }
  if (lightboxOpen()){
    import('./training-weergave.js?v=20260902b').then(m => m.sluitLightbox());
    return true;
  }
  if (pdfViewerOpen()){
    import('./pdf-viewer.js?v=20260902b').then(m => m.sluitPdfViewer());
    return true;
  }
