...
function _navTerug(scherm){
  import('./tracker.js?v=20260822a').then(m => m.telNav?.(scherm, 'terug')).catch(()=>{});
}

/* Eén terug-stap volgens prioriteit. true = afgehandeld (app blijft open). */
function stapTerug(){
  if (lightboxOpen()){
    import('./training-weergave.js?v=20260822a').then(m => m.sluitLightbox());
    return true;
  }
  if (pdfViewerOpen()){
    import('./pdf-viewer.js?v=20260822a').then(m => m.sluitPdfViewer());
    return true;
  }
