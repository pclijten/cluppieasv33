...
import { openTeam, zetTeamTab, verlaatTeamView, updatesInfo, renderTeam } from './teams.js?v=20260925a';
import { initSchermen, ruimOp } from './desktop-schermen.js?v=20260925a';
import { openBouw, bouwTeams, bouwHuidig, zetHerteken } from './desktop-bouw.js?v=20260925a';
...
async function sluitOpenWedstrijd(){
  ...
  const w = await import('./wedstrijd.js?v=20260925a');
  w.sluitWedstrijd();
  await import('./teams.js?v=20260925a');
...
async function naarOverzicht(){
  ...
  if (huidigeView() === 'club'){ const c = await import('./club.js?v=20260925a'); c.verlaatClubView(); }
}
async function openClubDesk(id){
  await sluitOpenWedstrijd();
  if (S.teamId) verlaatTeamView();
  const c = await import('./club.js?v=20260925a');
  c.openClub(id);
}
async function openBouwDesk(clubId, bouw){
  const b = await import('./bouw-hub.js?v=20260925a');
...
async function voerActieUit(actie, data = {}){
  ...
  if (actie === 'chat')       return import('./chatbot.js?v=20260925a').then(m => m.openChatbot());
  if (actie === 'tactiek'){
  ...
  if (actie === 'wedstrijd'){
    ...
    return import('./wedstrijd.js?v=20260925a').then(m => m.openWedstrijd(data.id));
  }
}
