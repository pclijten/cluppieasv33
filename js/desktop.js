...
import { openTeam, zetTeamTab, verlaatTeamView, updatesInfo, renderTeam } from './teams.js?v=20260925b';
import { initSchermen, ruimOp } from './desktop-schermen.js?v=20260925b';
import { openBouw, bouwTeams, bouwHuidig, zetHerteken } from './desktop-bouw.js?v=20260925b';
...
async function sluitOpenWedstrijd(){
  ...
  const w = await import('./wedstrijd.js?v=20260925b');
  w.sluitWedstrijd();
  await import('./teams.js?v=20260925b');
...
async function naarOverzicht(){
  ...
  if (huidigeView() === 'club'){ const c = await import('./club.js?v=20260925b'); c.verlaatClubView(); }
}
async function openClubDesk(id){
  await sluitOpenWedstrijd();
  if (S.teamId) verlaatTeamView();
  const c = await import('./club.js?v=20260925b');
  c.openClub(id);
}
async function openBouwDesk(clubId, bouw){
  const b = await import('./bouw-hub.js?v=20260925b');
...
async function voerActieUit(actie, data = {}){
  ...
  if (actie === 'chat')       return import('./chatbot.js?v=20260925b').then(m => m.openChatbot());
  if (actie === 'tactiek'){
  ...
  if (actie === 'wedstrijd'){
    ...
    return import('./wedstrijd.js?v=20260925b').then(m => m.openWedstrijd(data.id));
  }
}
