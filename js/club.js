...
import { CATEGORIEEN, CATEGORIEEN_MEIDEN, catInfo, BOUWEN, bouwVanCategorie, bouwNaam, youtubeId, youtubeThumb, youtubeWatch, SEIZOEN_FALLBACK, GEBRUIK_CATEGORIEEN, gebruikEventLabel } from './config.js?v=20260828d';
...
async function teamsModule(){ return await import('./teams.js?v=20260830a'); }
...
export function verlaatClubView(){
  ...
  import('./teams.js?v=20260830a').then(m => { m.renderTeams(); toon('teams'); });
}
...
function koppelClubTab(v, tab, teams, trainingen, videos, documenten){
  ...
  if (tab === 'trainingen'){
    ...
    v.querySelectorAll('[data-tnotities]').forEach(b => b.onclick = async () => {
      ...
      if (!t) return;
      const { openBeheerNotities } = await import('./training-notities-beheer.js?v=20260828d');
    ...
    v.querySelectorAll('[data-ttekst]').forEach(b => b.onclick = async () => {
      ...
      const { openTrainingBewerken } = await import('./training-bewerken.js?v=20260828d');
...
async function startTrainingVerwerking(file, meta){
  ...
  try {
    const ai = await import('./training-ai.js?v=20260828d');
