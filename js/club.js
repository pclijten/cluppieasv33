...
import { CATEGORIEEN, CATEGORIEEN_MEIDEN, catInfo, BOUWEN, bouwVanCategorie, bouwNaam, youtubeId, youtubeThumb, youtubeWatch, SEIZOEN_FALLBACK, GEBRUIK_CATEGORIEEN, gebruikEventLabel } from './config.js?v=20260828a';
import { analyseWedstrijd } from './analyse.js?v=20260828a';
import { clubEvaluatiesOphalen, htmlClubEvaluaties, koppelClubEvaluaties } from './club-evaluaties.js?v=20260828a';
import { startClubContentListener, htmlClubContent, koppelClubContent } from './club-content.js?v=20260828a';
...
async function teamsModule(){ return await import('./teams.js?v=20260828a'); }
...
export function verlaatClubView(){
  ...
  import('./teams.js?v=20260828a').then(m => { m.renderTeams(); toon('teams'); });
}
...
function koppelClubTab(v, tab, teams, trainingen, videos, documenten){
  ...
  if (tab === 'trainingen'){
    ...
    v.querySelectorAll('[data-tnotities]').forEach(b => b.onclick = async () => {
      ...
      if (!t) return;
      const { openBeheerNotities } = await import('./training-notities-beheer.js?v=20260828a');
    ...
    v.querySelectorAll('[data-ttekst]').forEach(b => b.onclick = async () => {
      ...
      const { openTrainingBewerken } = await import('./training-bewerken.js?v=20260828a');
...
async function startTrainingVerwerking(file, meta){
  ...
  try {
    const ai = await import('./training-ai.js?v=20260828a');
