...
import { CATEGORIEEN, CATEGORIEEN_MEIDEN, catInfo, BOUWEN, bouwVanCategorie, bouwNaam, youtubeId, youtubeThumb, youtubeWatch, SEIZOEN_FALLBACK, GEBRUIK_CATEGORIEEN, gebruikEventLabel } from './config.js?v=20260818b';
import { analyseWedstrijd } from './analyse.js?v=20260818b';
import { clubEvaluatiesOphalen, htmlClubEvaluaties, koppelClubEvaluaties } from './club-evaluaties.js?v=20260818b';
import { startClubContentListener, htmlClubContent, koppelClubContent } from './club-content.js?v=20260818b';
...
async function teamsModule(){ return await import('./teams.js?v=20260818b'); }
...
export function verlaatClubView(){
  ...
  import('./teams.js?v=20260818b').then(m => { m.renderTeams(); toon('teams'); });
}
...
function koppelClubTab(v, tab, teams, trainingen, videos, documenten){
  ...
  if (tab === 'trainingen'){
    ...
    v.querySelectorAll('[data-ttekst]').forEach(b => b.onclick = async () => {
      ...
      const { openTrainingBewerken } = await import('./training-bewerken.js?v=20260818b');
...
async function startTrainingVerwerking(file, meta){
  ...
  try {
    const ai = await import('./training-ai.js?v=20260818b');
