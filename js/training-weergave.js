...
import { bewaakTerug, vangnetStilTerugAlsNodig, esc } from './state.js?v=20260828b';
...
export function sluitTrainingWeergave(){
  ...
  if (nb) nb.remove();
  import('./training-aantekeningen.js?v=20260828b').then(m => m.resetAantekeningen()).catch(() => {});
  import('./training-video.js?v=20260828b').then(m => m.resetTrainingVideos()).catch(() => {});
...
export function openTrainingWeergave({ titel, meta, oefeningen, diagramUrls, onOrigineel, trainingId, oefeningVideos, trainingClub }){
  ...
  if (trainingId){
    import('./training-aantekeningen.js?v=20260828b').then(mod => {
    ...
    import('./training-video.js?v=20260828b').then(mod => {
      const balk = el.querySelector('.trw-balk');
