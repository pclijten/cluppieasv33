...
import { kompasTips, startContentListener } from './content.js?v=20260819c';
...
/* ---------- Submodules (teams.js-modulaire split) ----------
   ...
   gebruiken ze `import('./teams.js?v=20260819c')` binnen de aanroepende functie,
...
} from './teams-spelers.js?v=20260819c';
import { htmlKompas, toonThemaInfo, toonKompasInfo, kompasItems, kompasStartIndex } from './teams-leerlijn.js?v=20260819c';
import { modalTeamEvaluatie, htmlStatsTab, htmlTeamEvaluatieDashboard, htmlSeizoenFilter } from './teams-evaluatie.js?v=20260819c';
import {
...
} from './teams-training.js?v=20260819c';
import { htmlTeamDocumenten } from './teams-documenten.js?v=20260819c';
import {
...
} from './teams-hub.js?v=20260819c';
import { htmlHandleiding } from './teams-handleiding.js?v=20260819c';
import { koppelOnboardingHerstart } from './onboarding.js?v=20260819c';
import { htmlBerichtBalk, koppelBerichtBalk, htmlBerichtenArchief, ongelezenBerichten } from './berichten.js?v=20260819c';
...
export function renderTeams(){
  ...
  v.querySelectorAll('[data-open-club]').forEach(b => b.onclick = () => import('./club.js?v=20260819c').then(m => m.openClub(b.dataset.openClub)));
  const nt = v.querySelector('#nieuwTeam'); if (nt) nt.onclick = () => modalNieuwTeam();
  v.querySelector('#joinTeam').onclick = modalJoinTeam;
  const nc = v.querySelector('#nieuwClub'); if (nc) nc.onclick = () => import('./club.js?v=20260819c').then(m => m.modalNieuwClub());
...
export function modalNieuwTeam(clubId = null){
  ...
  $('#mTeamOk').onclick = async () => {
    ...
    sluitModal();
    if (clubT) import('./club.js?v=20260819c').then(m => m.openClub(clubT.id));
...
function koppelTeamTab(v, tab){
  if (tab === 'hub'){
    ...
    if (chatKnop) chatKnop.onclick = () =>
      import('./chatbot.js?v=20260819c').then(m => m.openChatbot());
  ...
  if (tab === 'documenten'){
    v.querySelectorAll('[data-open-document]').forEach(r => r.onclick = async () => {
      ...
      const { openPdfViewer } = await import('./pdf-viewer.js?v=20260819c');
      openPdfViewer({
        url: r.dataset.url,
  ...
  if (tab === 'trainingen' || tab === 'presentietraining'){
    ...
    v.querySelectorAll('[data-open-training]').forEach(r => r.onclick = async () => {
      ...
      if (Array.isArray(t?.oefeningen) && t.oefeningen.length){
        const { openTrainingWeergave } = await import('./training-weergave.js?v=20260819c');
  ...
  if (tab === 'instellingen'){
    ...
    v.querySelector('#deelLink').onclick = () => import('./club.js?v=20260819c').then(m => m.modalUitnodig(S.team));
