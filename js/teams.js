...
import { kompasTips, startContentListener } from './content.js?v=20260828b';
...
import { doSignOut, joinMetCode, zorgClubLidmaatschap } from './auth.js?v=20260828b';
...
} from './wedstrijd.js?v=20260828b';
import { openSjabloonScherm, luisterSjablonen } from './opstelling-sjabloon.js?v=20260828b';
...
/* ---------- Submodules (teams.js-modulaire split) ----------
   ...
   gebruiken ze `import('./teams.js?v=20260828b')` binnen de aanroepende functie,
...
} from './teams-spelers.js?v=20260828b';
import { htmlKompas, toonThemaInfo, toonKompasInfo, kompasItems, kompasStartIndex } from './teams-leerlijn.js?v=20260828b';
import { modalTeamEvaluatie, htmlStatsTab, htmlTeamEvaluatieDashboard, htmlSeizoenFilter } from './teams-evaluatie.js?v=20260828b';
import {
...
} from './teams-training.js?v=20260828b';
import { htmlTeamDocumenten } from './teams-documenten.js?v=20260828b';
import {
...
} from './teams-hub.js?v=20260828b';
import { htmlHandleiding } from './teams-handleiding.js?v=20260828b';
import { koppelOnboardingHerstart } from './onboarding.js?v=20260828b';
import { htmlBerichtBalk, koppelBerichtBalk, htmlBerichtenArchief, ongelezenBerichten } from './berichten.js?v=20260828b';
...
/* Publieke re-exports: consumenten van teams.js (main.js, wedstrijd.js, ...)
   importeren deze twee nog altijd via './teams.js?v=20260828b' — ze wonen nu fysiek in
  ...
  S.unsub.teams = onSnapshot(q1, snap => {
    ...
    if (!S._autoTeamGeprobeerd && !S.teamId && !S.clubId && S.teams.length){
      ...
      if (positie && positie.teamId === doelTeam.id && positie.wedstrijdId){
        import('./wedstrijd.js?v=20260828b').then(m => m.openWedstrijd(positie.wedstrijdId));
      }
    }
...
export function renderTeams(){
  ...
  v.querySelectorAll('[data-open-club]').forEach(b => b.onclick = () => import('./club.js?v=20260828b').then(m => m.openClub(b.dataset.openClub)));
  const nt = v.querySelector('#nieuwTeam'); if (nt) nt.onclick = () => modalNieuwTeam();
  v.querySelector('#joinTeam').onclick = modalJoinTeam;
  const nc = v.querySelector('#nieuwClub'); if (nc) nc.onclick = () => import('./club.js?v=20260828b').then(m => m.modalNieuwClub());
...
export function modalNieuwTeam(clubId = null){
  ...
  $('#mTeamOk').onclick = async () => {
    ...
    sluitModal();
    if (clubT) import('./club.js?v=20260828b').then(m => m.openClub(clubT.id));
...
function koppelTeamTab(v, tab){
  if (tab === 'hub'){
    ...
    if (chatKnop) chatKnop.onclick = () =>
      import('./chatbot.js?v=20260828b').then(m => m.openChatbot());
  ...
  if (tab === 'documenten'){
    v.querySelectorAll('[data-open-document]').forEach(r => r.onclick = async () => {
      ...
      const { openPdfViewer } = await import('./pdf-viewer.js?v=20260828b');
      openPdfViewer({
        url: r.dataset.url,
  ...
  if (tab === 'trainingen' || tab === 'presentietraining'){
    ...
    v.querySelectorAll('[data-open-training]').forEach(r => r.onclick = async () => {
      ...
      if (Array.isArray(t?.oefeningen) && t.oefeningen.length){
        const { openTrainingWeergave } = await import('./training-weergave.js?v=20260828b');
  ...
  if (tab === 'instellingen'){
    ...
    v.querySelector('#deelLink').onclick = () => import('./club.js?v=20260828b').then(m => m.modalUitnodig(S.team));
