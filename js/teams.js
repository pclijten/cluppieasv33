...
import {
  CATEGORIEEN, CATEGORIEEN_MEIDEN, catInfo, bouwVanCategorie, BOUWEN,
...
} from './wedstrijd.js?v=20260925d';
import { openSjabloonScherm, luisterSjablonen } from './opstelling-sjabloon.js?v=20260925d';
...
/* ---------- Submodules (teams.js-modulaire split) ----------
   ...
   gebruiken ze `import('./teams.js?v=20260925d')` binnen de aanroepende functie,
...
} from './teams-spelers.js?v=20260925d';
import { htmlKompas, toonThemaInfo, toonKompasInfo, kompasItems, kompasStartIndex } from './teams-leerlijn.js?v=20260922c';
import { modalTeamEvaluatie, htmlStatsTab, htmlTeamEvaluatieDashboard, htmlSeizoenFilter } from './teams-evaluatie.js?v=20260925d';
import {
...
} from './teams-training.js?v=20260922c';
...
import { htmlHandleiding } from './teams-handleiding.js?v=20260925d';
import {
...
import { koppelOnboardingHerstart } from './onboarding.js?v=20260925d';
...
/* Publieke re-exports: consumenten van teams.js (main.js, wedstrijd.js, ...)
   importeren deze twee nog altijd via './teams.js?v=20260925d' — ze wonen nu fysiek in
...
export function startTeams(){
  stopUnsubs('teams','clubs','gelezen','coord_onder','coord_midden','coord_boven','coord_eigen');
  ...
  S.unsub.teams = onSnapshot(q1, snap => {
    ...
    if (!S._autoTeamGeprobeerd && !S.teamId && !S.clubId && S.teams.length){
      ...
      const HERSTELBARE_TABS = new Set([
        ...
        'berichten','leerlijnoverzicht','meer','help','updates','instellingen','lijstjes',
      ]);
      ...
      if (positie && positie.teamId === doelTeam.id && positie.wedstrijdId){
        import('./wedstrijd.js?v=20260925d').then(m => m.openWedstrijd(positie.wedstrijdId));
      }
    }
  ...
  // [20260918] Bouwcoördinator-toegang: los van S.teams (waar je alleen in
  // zit als je zelf coach bent), 3 losse queries — één per bouw — zodat ook
  // een coördinator die zelf geen team coacht meteen zijn bouw(en) ziet.
  // Firestore ondersteunt geen OR over verschillende mapkeys in één query,
  // vandaar drie kleine losse listeners i.p.v. één grote.
  S.coordinatorBouwen = S.coordinatorBouwen || [];
  BOUWEN.forEach(b => {
    const qb = query(collection(db,'clubs'), where(`bouwCoordinatoren.${b.id}.${S.user.uid}`, '==', true));
    S.unsub['coord_'+b.id] = onSnapshot(qb, snap => {
      S.coordinatorBouwen = S.coordinatorBouwen.filter(x => x.bouw !== b.id)
        .concat(snap.docs.map(d => ({clubId:d.id, clubNaam:d.data().naam, bouw:b.id, bouwNaam:b.naam})));
      if (!S.teamId && !S.clubId) renderTeams();
    }, meldFout('coordinator-'+b.id));
  });
  /* [20260923e] Coach gekoppeld aan een EIGEN bouw (clubs.eigenBouwen[].coaches).
     Het clubdocument houdt daarvoor een platte lijst eigenBouwCoaches bij,
     zodat één array-contains-query volstaat. */
  const qe = query(collection(db,'clubs'), where('eigenBouwCoaches', 'array-contains', S.user.uid));
  S.unsub.coord_eigen = onSnapshot(qe, snap => {
    const extra = [];
    snap.docs.forEach(d => {
      for (const eb of (Array.isArray(d.data().eigenBouwen) ? d.data().eigenBouwen : []))
        if (eb?.id && eb.coaches?.[S.user.uid]) extra.push({clubId:d.id, clubNaam:d.data().naam, bouw:eb.id, bouwNaam:eb.naam || 'Bouw', eigen:true});
    });
    S.coordinatorBouwen = S.coordinatorBouwen.filter(x => !x.eigen).concat(extra);
    if (!S.teamId && !S.clubId) renderTeams();
  }, meldFout('coordinator-eigen'));
...
export function renderTeams(){
  ...
  v.innerHTML = `
    ...
    ${S.coordinatorBouwen.length ? `<div class="sectie-kop" style="margin-top:4px">Bouwen die je coördineert</div>
      ${S.coordinatorBouwen.map(b => `
        <button class="lijst-item" data-open-bouw="${b.clubId}|${b.bouw}">
          <div class="club-shirt"><img src="icons/asv-schild.png" alt="" class="club-schild"></div>
          <div class="li-tekst"><div class="titel">${esc(b.bouwNaam)} <span class="club-badge">coördinator</span></div>
          <div class="meta">${esc(b.clubNaam || 'club')}</div></div>
          <span class="pijl">›</span>
        </button>`).join('')}` : ''}
  ...
  v.querySelectorAll('[data-open-club]').forEach(b => b.onclick = () => import('./club.js?v=20260925d').then(m => m.openClub(b.dataset.openClub)));
  v.querySelectorAll('[data-open-bouw]').forEach(b => b.onclick = () => {
    const [clubId, bouw] = b.dataset.openBouw.split('|');
    import('./bouw-hub.js?v=20260925d').then(m => m.openBouwHub(clubId, bouw));
  });
  const nt = v.querySelector('#nieuwTeam'); if (nt) nt.onclick = () => modalNieuwTeam();
  v.querySelector('#joinTeam').onclick = modalJoinTeam;
  const nc = v.querySelector('#nieuwClub'); if (nc) nc.onclick = () => import('./club.js?v=20260925d').then(m => m.modalNieuwClub());
...
export function modalNieuwTeam(clubId = null){
  ...
  $('#mTeamOk').onclick = async () => {
    ...
    sluitModal();
    if (clubT) import('./club.js?v=20260925d').then(m => m.openClub(clubT.id));
...
export function openTeam(teamId, beginTab = 'hub', opties = {}){
  ...
  // [20260918] Vanuit de bouw-hub (coördinator) kan een speler direct naar
  // zijn profiel navigeren — maar S._beoordeelProfiel pas zetten zodra de
  // spelers-listener daadwerkelijk data heeft, anders valt htmlProfiel()'s
  // eigen guard er meteen weer overheen (zie de spelers-listener hieronder).
  S._pendingBeoordeelProfiel = opties.beoordeelProfiel || null;
  ...
  stopUnsubs('team','spelers','wedstrijden','presentie','planning','poule','beoordelingen','teamevaluaties','seizoen','sjablonen','uitleningen','uitleningenIn','lijsten');
  ...
  S.unsub.spelers = onSnapshot(collection(db,'teams',teamId,'spelers'), snap => {
    ...
    if (S._pendingBeoordeelProfiel){
      ...
      if (speler(pid)) { S._beoordeelProfiel = pid; S.teamTab = 'spelers'; }
    }
  ...
  // Opstelling-sjablonen (herbruikbare opstellingen per team). Eigen listener
  // zodat het sjabloonscherm en de "invoegen"-knop in de wedstrijd realtime
  // dezelfde lijst zien.
  S.unsub.sjablonen = luisterSjablonen(teamId);
  // [20260924a] Lijstjes (kledingmaten, aanwezigheid, ouderhulp). Ook op de hub
  // (badge) en de Planning (lijsten met datum) nodig, dus daar ook hertekenen.
  S._lijstOpen = null;
  S.unsub.lijsten = luisterLijsten(teamId, () => {
    if (!S.wedstrijdId && ['lijstjes','lijst','hub','planning'].includes(S.teamTab)) renderTeam();
  }, luisterfout('lijsten'));
  toon('team');
...
/* ==================== herbouwSpelers — de merge bij de bron ====================
   ...
   1. Eigen spelers (teams/{teamId}/spelers) MINUS gast-documenten (die
      krijgen hun eigen behandeling hieronder). Wie is uitgeleend blijft
      gewoon in deze bruikbare lijst staan — hij krijgt _uitgeleendAan
      (teamnaam) + _leenIdUit mee, zodat htmlSpelers een chip "⇄ ook bij
      {team}" kan tonen. Geen blokkade: dekt het geval waarin dezelfde
      speler dezelfde dag voor beide teams speelt.
...
function herbouwSpelers(){
  ...
  // uitgaande uitlening per speler-id (informatief — blokkeert niet meer).
  // Een speler kan dezelfde dag bij zijn eigen team én bij het lenende team
  // spelen (bv. 9:00 bij ons, 11:00 elders); de coach ziet dat via het label
  // bij zijn naam en regelt zelf of het qua tijd past.
  const uitgeleendPerId = new Map(uitleningenUit.map(u => [u.spelerId, u]));
  ...
  // 1) eigen spelers (geen gast) — ook wie is uitgeleend blijft gewoon bruikbaar
  for (const p of eigenDocs){
    if (p.gast) continue;
    const u = uitgeleendPerId.get(p.id);
    lijst.push(u
      ? { ...p, _ingeleend:false, _uitgeleendAan: u.naarTeamNaam, _leenIdUit: u.id }
      : { ...p, _ingeleend:false });
  }
  ...
  for (const u of uitleningenIn){
    ...
    lijst.push({
      ...
      // Een ingeleende speler heeft geen eigen spelerdocument bij dit team,
      // dus zijn instroomdatum leeft op het leen-record (zie js/opkomst.js).
      meetelVanaf: u.overlay?.meetelVanaf ?? null,
      _ingeleend: true,
      _bronTeam: u.vanTeam,
  ...
  for (const p of eigenDocs){
    ...
    if (u){
      ...
      lijst.push({
        ...
        meetelVanaf: u.overlay?.meetelVanaf ?? p.meetelVanaf ?? null,
...
export function verlaatTeamView(){
  stopUnsubs('team','spelers','wedstrijden','presentie','planning','poule','beoordelingen','uitleningen','uitleningenIn','teamevaluaties','seizoen','sjablonen','lijsten');
  S._teamTabStack = [];
...
const UPDATES = [
   { datum:'2026-09-24', titel:'Lijstjes: compacter en met eigen namen', punten:[
    ...
    'Namen aanvullen doe je onderaan de lijst, hernoemen of weghalen via ⋯ → Namen beheren.',
    'Het tandwiel rechtsboven is weg. De instellingen van je team vind je via de tegel Instellingen onder Meer.',
  ]},
  { datum:'2026-09-24', titel:'Nieuw: Lijstjes', punten:[
    'Onder Meer vind je nu Lijstjes: handig voor alles wat je voor het team bijhoudt, zoals wie er meegaat naar een toernooi, welke kledingmaat iedereen heeft of welke ouder rijdt.',
    'Kies een sjabloon (Aanwezigheid, Kledingmaten, Ouderhulp) of begin leeg. Je selectie staat er meteen in; nieuwe spelers komen er vanzelf bij.',
    'Kledingmaten: 116 t/m 164 en S t/m XL staan klaar. Tik op een maat om zelf een maat toe te voegen, bijvoorbeeld 176 of XXL. Bovenaan zie je per maat hoeveel je nodig hebt.',
    'Ouderhulp: de rijen zijn je wedstrijden. Bij thuiswedstrijden zijn geen rijders nodig.',
    'Geef een lijst een datum, dan verschijnt hij ook in de Planning.',
    'Elke lijst kun je delen via WhatsApp. Je kiest zelf welke kolommen in het bericht komen, en er staan alleen voornamen in.',
    'Zet geen medische gegevens (allergieën, medicijnen, blessures) in een lijst.',
  ]},
  { datum:'2026-09-23', titel:'Bouw-omgeving op de telefoon vernieuwd', punten:[
    'Het bouw-dashboard op de telefoon heeft nu dezelfde opbouw als op de laptop: onderin Dashboard, Teams en Uitleningen.',
    'Per team kies je bovenin Selectie, Evaluatie spelers, Evaluatie wedstrijden of Stats. Bij je eigen team open je met één tik de gewone, bewerkbare schermen.',
    'Rechtsboven wissel je snel van bouw, ook naar een eigen bouw zoals de Meidenbouw.',
  ]},
  { datum:'2026-09-23', titel:'Bouwen: coaches koppelen, staf en uitleningen', punten:[
    'Aan een eigen bouw (zoals de Meidenbouw) kun je coaches koppelen. Zij krijgen dezelfde rechten als een bouwcoördinator.',
    'Bouwcoördinatoren die geen clubbeheerder zijn kregen soms "geen toegang" op het bouw-dashboard. Opgelost.',
    'Op de laptop zie je bij Selectie de staf van het team, en bij uitleningen of een speler binnen de bouw, naar een andere bouw of uit een andere bouw komt.',
  ]},
  { datum:'2026-09-23', titel:'Eigen bouwen, zoals een Meidenbouw', punten:[
    'Clubbeheerders kunnen in Club \u2192 Instellingen eigen bouwen maken, bijvoorbeeld een Meidenbouw, en daar teams aan koppelen. Een team mag in meerdere bouwen zitten.',
    'Eigen bouwen staan op de laptop in het menu onder Club \u00b7 beheer, met hetzelfde dashboard, dezelfde uitleningen en dezelfde teamschermen.',
    'Oefenstof en coördinatoren blijven bij de standaardbouw (onder-, midden- of bovenbouw).',
  ]},
  { datum:'2026-09-23', titel:'Desktop: speler bewerken en de bouw-omgeving', punten:[
    'Klik je in Selectie op een speler, dan bewerk je direct zijn gegevens, voorkeurspositie en notitie, en kun je hem uitlenen aan een ander team.',
    'Onder Club \u00b7 beheer klapt elke bouw uit: een dashboard met alle teams (uitslagen, komende wedstrijden, opkomst, uitleningen), een overzicht van de uitleningen en per team Selectie, Evaluatie spelers, Evaluatie wedstrijden en Stats.',
    'Bij teams waar je geen coach van bent kijk je mee: alleen lezen.',
  ]},
  { datum:'2026-09-23', titel:'Desktop: planning als kalender en bouw-dashboards in het menu', punten:[
    'Planning toont op de laptop een maandkalender met wedstrijden, trainingen en speeldagen. Klik op een dag om hem te openen of aan te passen.',
    'Clubbeheerders vinden in het menu links onder Club · beheer de dashboards van de onder-, midden- en bovenbouw en de plek om coördinatoren toe te wijzen.',
  ]},
  ...
  { datum:'2026-09-22', titel:'Cluppie op de laptop: nieuwe desktopweergave', punten:[
    ...
    'Spelers als kaarten met een cijfer per domein, en per speler een paspoort met radar, leerpunten en laatste momenten. De cijfers zijn alleen voor coaches zichtbaar.',
    'Wedstrijden, trainingen en de leerlijn krijgen op de laptop een overzicht met lijst links en details rechts. Via "Gewone weergave" kom je altijd bij het vertrouwde scherm met alle opties.',
    'Op je telefoon verandert er niets.',
  ]},
  { datum:'2026-09-22', titel:'Bouwcoördinatoren: eigen dashboard per bouw', punten:[
    'Nieuwe rol voor wie meerdere teams binnen een bouw (onder/midden/boven) overziet. Een clubadmin wijst coördinatoren aan onder Instellingen.',
    'Een coördinator krijgt op het startscherm een eigen dashboard voor zijn bouw: een teamkaart per team met poulestand, vorm, presentie bij training én wedstrijden, en evaluatie-voortgang.',
    'Vanuit hetzelfde dashboard: spelers over de hele bouw (positie, opkomst, cijfer), uitleningen tussen teams (incl. zelf een speler uitlenen), en alle evaluaties per team.',
    'Welke onderdelen op elke teamkaart staan, stel je zelf in via het tandwiel rechtsboven op het dashboard.',
  ]},
  { datum:'2026-09-22', titel:'Training delen met een ouder', punten:[
    'Helpt er een ouder mee bij de training? Deel de training via het knopje rechtsboven in de oefenstof-weergave.',
    'Kies hoelang de link geldig is; de ouder kan — zonder eigen account — de training bekijken én zien wie er is (presentie, alleen-lezen). Verder krijgt hij nergens toegang toe.',
  ]},
  { datum:'2026-09-21', titel:'Presentie: twee kleine, merkbare verbeteringen', punten:[
    'De eerstvolgende training staat nu altijd apart bovenaan het presentie-tabblad, los van de geschiedenis — voorheen kon een vooraf ingevulde afmelding voor een latere datum boven de training van vanavond staan.',
    'Bij het afmelden van een speler kun je nu bij élke reden een toelichting typen, niet meer alleen bij "Anders".',
  ]},
  { datum:'2026-09-08', titel:'Trainingsopkomst klopt nu ook voor spelers die later instroomden', punten:[
    'Een speler die halverwege het seizoen bij het team komt — een gast, een inleen of een nieuwe aanwinst — kreeg tot nu toe 100% opkomst over alle trainingen van vóór zijn komst. Hij was er simpelweg niet om afwezig gezet te worden. Vanaf nu legt de app bij elke presentie vast wie er die dag in de selectie zat, en telt zijn percentage alleen over de trainingen waar hij bij hoorde.',
    'Onder zijn naam staat een chip die vertelt hoeveel trainingen buiten zijn telling vielen, zodat een stand van 1 / 1 niet als registratiefout leest.',
    'Het aandacht-blad signaleert lage opkomst pas vanaf drie meetellende trainingen. Eén gemiste training van iemand die net binnen is, is geen patroon.',
    'Ook het opkomstcijfer op het clubdashboard, het spelersprofiel en de Excel-export rekenen op deze manier — die liepen anders uit elkaar.'],
  },
  { datum:'2026-09-07', titel:'Stats overzichtelijker + geleende spelers meteen op de bank', punten:[
      'De tabbladen Speel en Wed zijn samengevoegd tot één blad Spelers: speeltijd, wedstrijden, reserve, doelpunten, aanvoerder, keeper en kaarten staan nu naast elkaar in dezelfde tabel.',
      'Kolommen waarin het hele team op nul staat worden weggelaten — heeft niemand een kaart gekregen, dan zie je die kolommen ook niet. Een 0 in een kolom die verder wel gevuld is, staat gedempt.',
      'De tabel staat nu op naam gesorteerd, zodat je een speler altijd op dezelfde plek terugvindt. Spelers onder de 50% speeltijd krijgen een lichte tint.',
      'Gebruik je beoordelingen, dan staat achter elke naam een klein lijntje met zijn ontwikkeling: elke snelle beoordeling en het gemiddelde van elke volledige beoordeling, oudste links.',
      'Een speler die je van een ander team leent staat nu meteen in de selectie van nog niet gespeelde wedstrijden, in plaats van bij afwezig. Haal je hem er zelf uit, dan blijft dat zo.',
    ]},
  { datum:'2026-09-07', titel:'Speeltijd: alle kwarten in één tabel', punten:[
      'Onder “Speeltijd deze wedstrijd” zie je nu alle kwarten (of helften) naast elkaar in plaats van alleen het kwart waar je op staat. Zo zie je in één oogopslag hoe de speeltijd over de wedstrijd verdeeld is.',
      'Het kwart waar je nu in zit is rood gemarkeerd. De spelernaam en de totaalkolom blijven staan als je de tabel opzij schuift — handig bij een toernooi met veel periodes.',
      'Elke cel is aan te tikken om die periode voor die speler te corrigeren; voorheen kon dat alleen bij het actieve kwart.',
      'De aparte keeperkolom is vervangen door een oranje K achter de speeltijd, zodat je meteen ziet in wélke periodes iemand op doel stond.',
    ]},
  { datum:'2026-09-05', titel:'Speelminuten: twee telfouten hersteld', punten:[
      'Werd een speler gewisseld terwijl hij niet in de startopstelling van die periode stond — bijvoorbeeld omdat je de opstelling pas na het starten van de klok neerzette, of omdat je de wissel achteraf toevoegde — dan viel zijn gespeelde tijd in die periode volledig weg. Die tijd telt nu gewoon mee.',
      'Zette je de klok halverwege een periode op pauze en startte je hem niet meer, dan gold die klokstand als de lengte van de hele periode. Iedereen verloor daardoor minuten en een invaller kwam op 0:00 uit. De ingestelde periodeduur is nu de ondergrens; de klok kan een periode alleen nog verlengen bij blessuretijd.',
      'Beide correcties werken met terugwerkende kracht op al gespeelde wedstrijden. Speelminuten en eerlijkheidspercentages kunnen daardoor iets afwijken van wat je eerder zag — die nieuwe waarden zijn de juiste.',
    ]},
  ...
  { datum:'2026-08-28', titel:'Tactiekbord: teken je plan op het veld', punten:[
      ...
      'Zet “Volg-loopweg” aan en sleep een speler: de loopweg tekent zich vanzelf mee — handig om te laten zien waar iemand naartoe loopt.',
      'Je kunt meerdere tactiekborden per wedstrijd bewaren (bijvoorbeeld “K1 omschakeling” of “corner voor”), en een bord als afbeelding delen met je mede-coaches.',
    ]},
  { datum:'2026-08-26', titel:'Wijzig opzet slaat nu alles automatisch op', punten:[
      'In het scherm “Wijzig opzet” hoef je niet meer op Opslaan te tikken: elke keuze wordt meteen bewaard. Kies je een aanvoerder, aantal spelers, formatie of een doel-suggestie, dan staat het direct vast.',
      'Tekstvelden (tegenstander, datum, minuten, wedstrijddoel en notitie) worden opgeslagen zodra je uit het veld tikt. Bij elk opgeslagen veld verschijnt kort een groen “Opgeslagen”-vinkje, en onderaan zie je wat er als laatste is bewaard.',
      'De Opslaan-knop is vervangen door “Klaar”. Sluiten met het kruisje kan ook gewoon — je wijzigingen staan dan al veilig.',
    ]},
  { datum:'2026-08-26', titel:'Standaardopstellingen (sjablonen)', punten:[
      'Je kunt nu een vaste opstelling voor alle kwarten bewaren als sjabloon en die in één keer op een andere wedstrijd leggen — inclusief dezelfde spelers. Handig als je week na week met (grotendeels) dezelfde opstelling begint.',
      'Onder het tabblad Wedstrijden staat een nieuwe knop “Opstelling-sjablonen”. Daar maak je een sjabloon van de grond af op: kies aantal spelers en formatie, en tik per kwart de spelers op hun plek. Met “⧉ Naar K2/K3/K4” kopieer je een kwart razendsnel naar het volgende.',
      'Al een mooie opstelling gemaakt in een wedstrijd? Onderaan het wedstrijdscherm bewaar je die met één tik als sjabloon via “Opslaan als sjabloon”.',
      'Bij een nog lege wedstrijd verschijnt bovenaan de knop “Opstelling invoegen”: kies een sjabloon óf een vorige wedstrijd en alle kwarten worden in één keer gevuld. Spelers die niet in je selectie zitten worden netjes overgeslagen — je hoort achteraf hoeveel.',
      'Ook via “Wijzig opzet” (onder Speelwijze & formatie) kun je een standaardopstelling invoegen — óók als de wedstrijd al een opstelling heeft. Je krijgt eerst een bevestiging voordat de bestaande opstellingen worden vervangen.',
      'Sjablonen zijn per team: elk team beheert z’n eigen standaardopstellingen.',
    ]},
...
/* Bouwt de per-datum gegroepeerde speelronden uit de programmaregels.
   ...
   De regels zijn al tot de eigen poule beperkt door de sync (filtert op de
   teamnamen uit de poulestand), dus hier is geen extra filter meer nodig. */
...
export function renderTeam(){
  ...
  /* [20260922e] Desktop-schil: op schermen ≥ 1100 px tekent desktop-schermen.js
     een eigen weergave voor dashboard, selectie/paspoort, wedstrijden,
     trainingen en leerlijn. Geeft de hook false terug (smal scherm, ander
     tabblad, "Gewone weergave" of een fout), dan gaat alles hieronder
     ongewijzigd door. Op telefoon bestaat S._deskRenderTeam niet eens. */
  if (S._deskRenderTeam && S._deskRenderTeam(v, tab)) return;
  ...
  if (tab === 'lijstjes')    inhoud = htmlLijstjes();
  if (tab === 'lijst')       inhoud = htmlLijst();
  ...
  const TAB_TITEL = {
    ...
    help:'Help', updates:'Updates', lijstjes:'Lijstjes', lijst:lijstTitel(),
  };

  const profielOpen = (tab === 'spelers' && S._beoordeelProfiel);
  v.innerHTML = `
    ${profielOpen ? '' : `<div class="kop"><button class="terug" id="naarTeams">‹</button>
      <h1>${esc(TAB_TITEL[tab] || S.team.naam)}<span class="sub">${esc(S.team.naam)}</span></h1></div>`}
    ${inhoud}`;

  const naarTeamsBtn = v.querySelector('#naarTeams');
  if (naarTeamsBtn) naarTeamsBtn.onclick = () => history.back();
  /* [20260924b] Het ⚙️-tandwiel rechtsboven is weg: Instellingen zit in de hub-tegel (Meer). */
  koppelTeamTab(v, tab);
...
function htmlWedstrijden(){
  const knop = `<button class="knop vol" id="nieuweWedstrijd" style="margin-bottom:8px">+ Nieuwe wedstrijd</button>
    <button class="knop licht vol" id="sjabloonBeheer" style="margin-bottom:14px">⧉ Opstelling-sjablonen</button>`;
...
const PLAN_TYPE = {
  ...
  lijst:  {kort:'LIJST', klas:'lijst', naam:'Lijst'},
};
const PLAN_FILTERS = [
...
/* [20260923b] geëxporteerd voor de desktop-kalender (desktop-schermen.js). */
export function planningItems(){
  ...
  // [20260924a] lijstjes met een datum (bv. toernooi-aanwezigheid)
  try { items.push(...lijstPlanningItems()); } catch(e){ console.warn('[Cluppie] lijst-items planning', e); }
...
function koppelTeamTab(v, tab){
  if (tab === 'lijstjes' || tab === 'lijst'){ koppelLijstjes(v, tab, { zetTeamTab, renderTeam }); return; }
  if (tab === 'hub'){
    ...
    if (chatKnop) chatKnop.onclick = () =>
      import('./chatbot.js?v=20260925d').then(m => m.openChatbot());
  ...
  if (tab === 'documenten'){
    v.querySelectorAll('[data-open-document]').forEach(r => r.onclick = async () => {
      ...
      const { openPdfViewer } = await import('./pdf-viewer.js?v=20260922c');
      openPdfViewer({
        url: r.dataset.url,
  ...
  if (tab === 'planning'){
    ...
    v.querySelectorAll('[data-plandag]').forEach(b => b.onclick = () => {
      ...
      if (bron === 'lijst'){
        const lid = b.dataset.plandoc;
        if (lid) openLijst(lid, zetTeamTab);
        return;
      }
    ...
    v.querySelectorAll('[data-open-training]').forEach(r => r.onclick = async () => {
      ...
      if (Array.isArray(t?.oefeningen) && t.oefeningen.length){
        ...
        openTrainingWeergave({
          ...
          oefeningTactiek: t.oefeningTactiek || {},
          trainingClub: t.club || null,
          teamId: S.teamId,
          teamNaam: S.team?.naam || '',
          clubNaam: S.team?.clubNaam || '',
        });
      } else {
        await openOrigineel();
      }
  ...
  if (tab === 'wedstrijden'){
    ...
    const sjBtn = v.querySelector('#sjabloonBeheer');
    if (sjBtn) sjBtn.onclick = () => openSjabloonScherm();
  ...
  if (tab === 'spelers' && S._beoordeelProfiel){
    ...
    // Nest binnen de speler-rij (chip "⇄ terug"): klik mag niet doorbubbelen
    // naar de data-open-profiel-knop eromheen.
    v.querySelectorAll('[data-uitleen-terug]').forEach(b => b.onclick = (e) => { e.stopPropagation(); trekUitleningIn(b.dataset.uitleenTerug); });
  ...
  if (tab === 'instellingen'){
    ...
    v.querySelector('#deelLink').onclick = () => import('./club.js?v=20260925d').then(m => m.modalUitnodig(S.team));
