...
import { doSignOut, joinMetCode, zorgClubLidmaatschap } from './auth.js?v=20260811a';
import { tekenPwaBanner } from './pwa.js?v=20260811a';
import {
  openWedstrijd, modalNieuweWedstrijd, renderWedstrijd
} from './wedstrijd.js?v=20260812b';
...
/* ---------- Submodules (teams.js-modulaire split) ----------
   ...
   gebruiken ze `import('./teams.js?v=20260812b')` binnen de aanroepende functie,
...
} from './teams-spelers.js?v=20260812b';
import { htmlKompas, toonThemaInfo, toonKompasInfo } from './teams-leerlijn.js?v=20260811a';
import { modalTeamEvaluatie, htmlStatsTab } from './teams-evaluatie.js?v=20260812b';
import {
...
} from './teams-training.js?v=20260811b';
import { htmlTeamDocumenten } from './teams-documenten.js?v=20260811a';
import { htmlHandleiding } from './teams-handleiding.js?v=20260812b';
import { koppelOnboardingHerstart } from './onboarding.js?v=20260812b';
...
export function renderTeams(){
  ...
  v.querySelectorAll('[data-open-club]').forEach(b => b.onclick = () => import('./club.js?v=20260812b').then(m => m.openClub(b.dataset.openClub)));
  const nt = v.querySelector('#nieuwTeam'); if (nt) nt.onclick = () => modalNieuwTeam();
  v.querySelector('#joinTeam').onclick = modalJoinTeam;
  const nc = v.querySelector('#nieuwClub'); if (nc) nc.onclick = () => import('./club.js?v=20260812b').then(m => m.modalNieuwClub());
...
export function modalNieuwTeam(clubId = null){
  ...
  $('#mTeamOk').onclick = async () => {
    ...
    sluitModal();
    if (clubT) import('./club.js?v=20260812b').then(m => m.openClub(clubT.id));
...
function koppelTeamTab(v, tab){
  if (tab === 'meer'){
    ...
    if (chatKnop) chatKnop.onclick = () =>
      import('./chatbot.js?v=20260812b').then(m => m.openChatbot());
    return;
  }
  if (tab === 'documenten'){
  ...
  if (tab === 'instellingen'){
    ...
    v.querySelector('#deelLink').onclick = () => import('./club.js?v=20260812b').then(m => m.modalUitnodig(S.team));
