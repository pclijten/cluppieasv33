...
import { kwartGespeeld, effectieveLineup, analyseKwart, analyseWedstrijd, speeltijdReserve, disciplinaireTijd } from './analyse.js?v=20260922b';
import { ico } from './icons.js?v=20260922b';

import { telGebruik, telNav } from './tracker.js?v=20260922b';
import { opkomstVoor, teltMee, MIN_OPKOMST_TRAININGEN } from './opkomst.js?v=20260922b';
import { openInvoegSheet, bewaarWedstrijdAlsSjabloon, zetNaToepassenCallback } from './opstelling-sjabloon.js?v=20260922b';
...
export function openWedstrijd(wid){
  ...
  if (!S.teamId || !wid){
    ...
    S.wedstrijdId = null;
    if (S.teamId) import('./teams.js?v=20260922b').then(m => m.renderTeam?.());
    return;
  }
...
export function sluitWedstrijd(naarTab){
  ...
  bewaarPositie();
  import('./teams.js?v=20260922b').then(m => { m.renderTeam(); toon('team'); });
}
...
function modalKlokInstellen(){
  ...
  openModal(`
    ...
    <div class="veldgroep"><label>Klok zetten op</label>
      ...
      <p style="font-size:calc(12px * var(--fs));color:var(--ink-2);margin-top:8px;line-height:1.5">Nu op <b>${mmss(nu)}</b> · periode duurt ${String(w.kwartduur).replace('.',',')} min (blessuretijd mag hoger).</p></div>
  ...
  $('#mKiOk').onclick = () => {
    ...
    let doel = Math.max(0, min*60 + sec);
...
function modalSpeeltijdCorrigeren(pid, opties = {}){
  ...
  /* Vangnet: bij oude wedstrijden kan een periode-object ontbreken. */
  const k = w.kwarten[nr] || (w.kwarten[nr] = leegKwart());
...
/* Leest een min:sec-paar uit twee invoervelden. Alleen ondergrens 0 — geen
   bovengrens, zodat blessuretijd (bv. 40+3) vrij ingevuld kan worden. De
   speeltijdberekening (analyseKwart) telt zelf niet verder dan de periodeduur;
   dit is puur de tijdstempel van de gebeurtenis. */
function leesMinSec(minId, secId){
...
/* Toont een gebeurtenistijd in voetbalnotatie. Binnen de periodeduur gewoon
   mm:ss (bv. 12:30). Valt de tijd in de blessuretijd (voorbij de periodeduur),
   dan tonen we <duur>+<extra minuten> — bv. 40+3 bij een helft van 40 min, of
   15+2 bij een kwart van 15. De extra tijd rondt naar boven af op hele minuten,
   zodat 40:01 t/m 41:00 als 40+1 verschijnt. w = wedstrijd (voor kwartduur). */
function gebeurtenisTijd(sec, w){
  sec = Math.round(sec || 0);
  const duurMin = Math.round((w?.kwartduur || 0));
  const duurSec = duurMin * 60;
  if (!duurSec || sec <= duurSec) return mmss(sec);
  const extra = Math.ceil((sec - duurSec) / 60);
  return `${duurMin}+${extra}`;
}
...
export function toonBijwerkScherm(){
  ...
  const wisselRegel = e => {
    ...
    return `<div class="bijwerk-regel" data-bw-wissel="${e.i}">
      ...
      <span class="bw-tijd">${gebeurtenisTijd(e.sec, w)} <span class="bw-pen">✎</span></span></div>`;
  };
  ...
  const kaartRegel = c => `<div class="bijwerk-regel${c.auto?' bw-auto':''}" ${c.auto?'':`data-bw-kaart="${c.i}"`}>
      ...
      <span class="bw-tijd">${gebeurtenisTijd(c.sec, w)}${c.auto?'':' <span class="bw-pen">✎</span>'}</span></div>`;
...
function modalWisselAchteraf(startNr, eventIndex = null){
  ...
  const origineelNr = String(startNr);   // periode waar het bestaande event nu staat
  let nr = String(startNr);               // gekozen periode (kan wijzigen)

  /* Snapshot van het bestaande event vóór eventueel verplaatsen. */
  const ev = bestaand ? (w.kwarten[origineelNr]?.events||[])[eventIndex] : null;
  ...
  function teken(){
    ...
    openModal(`
      ...
      <div class="veldgroep"><label>Periode</label>${periodeKiesHtml(w, nr, 'bwWaPeriode')}</div>
      <div class="veldgroep"><label>Eruit (van het veld)</label>
      ...
      ${bestaand ? `<button class="knop gevaar vol" id="bwWaWeg" style="margin-bottom:8px">🗑 Wissel verwijderen</button>` : ''}
    ...
    bindPeriodeKies('bwWaPeriode', nr, (gek) => { nr = gek; teken(); });
    ...
    $('#bwWaOk').onclick = () => {
      ...
      /* slot bepalen aan de hand van de uitgaande speler in de effectieve
         opstelling van de gekozen periode; valt terug op het oude slot. */
      const l = effectieveLineup(k);
      const slot = (uit && Object.keys(l).find(s => l[s] === uit)) || (bestaand ? ev.slot : null);
      const nieuw = {in: inP, uit, slot, sec};
      if (reden) nieuw.reden = reden;
      if (bestaand){
        if (nr !== origineelNr){
          /* Verplaatst naar een andere periode: uit de oude events halen en in
             de nieuwe periode toevoegen. */
          (w.kwarten[origineelNr].events ||= []).splice(eventIndex, 1);
          (k.events ||= []).push(nieuw);
        } else {
      ...
      bewaarWedstrijd();
      meld(bestaand ? 'Wissel bijgewerkt' : `Wissel toegevoegd · ${gebeurtenisTijd(sec, w)}`);
    ...
    if (wegBtn) wegBtn.onclick = () => {
      ...
      S.bijwerkKwart = origineelNr; toonBijwerkScherm();
    };
...
function modalGoalCorrigeren(i, opties = {}){
  ...
  const g = (w.goals||[])[i];
  if (!g) return;
  let nr = g.kwart != null ? String(g.kwart) : S.kwart;
  const terug = () => { if (opties.terugNaarBijwerk){ S.bijwerkKwart = nr; toonBijwerkScherm(); } else { sluitModal(); renderWedstrijd(); } };

  function teken(){
    const g2 = (w.goals||[])[i]; if (!g2) { terug(); return; }
    const k = w.kwarten[nr] || huidigKwart();
    const l = effectieveLineup(k);
    const veldSpelers = Object.values(l).filter(pid => speler(pid));
    const overig = (w.selectie||[]).filter(pid => speler(pid) && !veldSpelers.includes(pid));
    const huidigeOmschrijving = g2.type === 'voor'
    ...
    openModal(`
      ...
      <p style="font-size:calc(13.5px * var(--fs));color:var(--ink-2);margin-bottom:14px">Nu geregistreerd als <b>${esc(huidigeOmschrijving)}</b> op ${gebeurtenisTijd(g2.sec, w)}.</p>
      <div class="veldgroep"><label>Periode</label>${periodeKiesHtml(w, nr, 'mGcPeriode')}</div>
      <div class="veldgroep" style="margin-bottom:12px"><label>Minuut in de periode</label>${minSecInvoerHtml('mGcMin','mGcSec', g2.sec||0)}
        <p style="font-size:calc(11.5px * var(--fs));color:var(--ink-2);margin-top:6px;line-height:1.5">Blessuretijd mag: bij een helft van ${String(w.kwartduur).replace('.',',')} min vul je 40+3 in als <b>43</b> min.</p></div>
      <div class="correctie-opties">
        ...
        `}
        <button class="knop licht vol" id="mGcKant">${g2.type === 'voor' ? '↔ Toch een tegendoelpunt' : '↔ Maak er een doelpunt vóór '+esc(S.team.naam)+' van'}</button>
        <button class="knop gevaar vol" id="mGcWeg" style="margin-top:8px">🗑 Doelpunt verwijderen</button>
        <button class="knop vol" id="mGcOk" style="margin-top:8px">Opslaan</button>
      </div>`);

    if (opties.terugNaarBijwerk) kruisNaarBijwerk(terug);
    /* Periodewissel: onthoud eerst de zojuist ingevulde scorer/minuut niet —
       we her-renderen met de nieuwe periode-context (andere veldspelers). */
    bindPeriodeKies('mGcPeriode', nr, (gek) => { nr = gek; teken(); });

    const bewaarBasis = () => {
      w.goals[i].sec = leesMinSec('#mGcMin','#mGcSec');
      w.goals[i].kwart = nr;
    };
    $('#mGcOk').onclick = () => {
      bewaarBasis();
...
function modalKaartCorrigeren(i, opties = {}){
  ...
  const c = (w.kaarten||[])[i];
  if (!c) return;
  let nr = c.kwart != null ? String(c.kwart) : S.kwart;
  const terug = () => { if (opties.terugNaarBijwerk){ S.bijwerkKwart = nr; toonBijwerkScherm(); } else { sluitModal(); renderWedstrijd(); } };
  ...
  openModal(`
    ...
    <p style="font-size:calc(13.5px * var(--fs));color:var(--ink-2);margin-bottom:14px">Nu: <b>${esc(KAART_NAAM[c.type])}</b> voor <b>${esc(spelerNaam(c.pid))}</b> op ${gebeurtenisTijd(c.sec, w)}.</p>
    <div class="veldgroep"><label>Periode</label>${periodeKiesHtml(w, nr, 'mKcPeriode')}</div>
    ...
    <div class="correctie-opties">
      ...
      <button class="knop vol" id="mKcOk" style="margin-top:8px">Opslaan</button>
    </div>`);
  if (opties.terugNaarBijwerk) kruisNaarBijwerk(terug);
  bindPeriodeKies('mKcPeriode', nr, (gek) => { nr = gek; });
  $('#mKcOk').onclick = () => {
    w.kaarten[i].pid = $('#mKcSpeler').value;
    w.kaarten[i].sec = leesMinSec('#mKcMin','#mKcSec');
    w.kaarten[i].kwart = nr;
...
function genereerVerslag(){
  ...
  if (heeftWissels){
    ...
    for (const nr of periodeNrs(w)){
      ...
      for (const e of [...k.events].sort((a,b)=>a.sec-b.sec)){
        const t = gebeurtenisTijd(e.sec, w);
  ...
  if ((w.kaarten||[]).filter(c => !c.auto).length){
    ...
    for (const c of [...w.kaarten].filter(c => !c.auto).sort((a,b)=>a.sec-b.sec)){
      ...
      lines.push(`• ${spelerNaam(c.pid)} — ${txt} (${lbl}, ${gebeurtenisTijd(c.sec, w)})`);
    }
    lines.push('');
  }
  return lines.join('\n').trim();
...
export function htmlStats(){
  ...
  /* Sortering: alfabetisch op naam. De tabel is een naslagtabel — een coach
     ...
     weinig speelt wordt gesignaleerd via de eerlijkheidskaart erboven, de tint
     op de rij en het Aandacht-blad — niet via de sorteervolgorde. */
  ...
  /* --- Ontwikkelreeks per speler, voor het lijntje achter de naam ---
     Bron: S.beoordelingen (teams/{teamId}/beoordelingen), dat al in de state
     staat — geen extra Firestore-lees. Een snelle beoordeling levert zijn
     niveau 1..5, een volledige het gemiddelde van de ingevulde domeinscores;
     beide op dezelfde schaal, zodat ze in één lijn passen. Oud → nieuw, want
     S.beoordelingen staat nieuwste eerst. */
  ...
  for (const b of [...(S.beoordelingen||[])].reverse()){
    ...
    if (b.soort === 'snel' && b.niveau) v = Number(b.niveau);
      ...
      const sc = Object.values(b.scores || {}).filter(n => Number(n) > 0).map(Number);
      if (sc.length) v = sc.reduce((a,n) => a+n, 0) / sc.length;
    }
  ...
  /* 'speel' en 'wed' waren twee losse bladen tot deze release; die waarde kan
     nog in S staan uit een lopende sessie. Beide wijzen nu naar het
     samengevoegde spelers-blad, zodat niemand op een leeg scherm belandt. */
  ...
  /* ==================== SPELERS-BLAD ====================
     ...
     krijgt geen twee kolommen vol nullen te zien. Een 0 in een kolom die wél
     getoond wordt staat gedempt, zodat het oog naar de gevulde cellen gaat. */
  const spelersBlad = () => {
    const mogelijk = [
...
export function koppelStatsBlad(root){
  (root || document).querySelectorAll('[data-statsblad]').forEach(b => b.onclick = () => {
    ...
    import('./teams.js?v=20260922b').then(m => m.renderTeam?.());
  });
}
...
${confroHtml}
  ...
  if (tbk) tbk.onclick = () => import('./tactiekbord.js?v=20260922b').then(m => m.openTactiekLijst(w));
  ...
  if (teamEvalKnop) teamEvalKnop.onclick = () => {
    import('./teams.js?v=20260922b').then(m => m.modalTeamEvaluatie(S.wedstrijdId));
  };
  ...
  v.querySelectorAll('[data-corrigeer-speeltijd]').forEach(td => td.onclick = e => {
    e.stopPropagation();
    modalSpeeltijdCorrigeren(td.dataset.corrigeerSpeeltijd, td.dataset.corrigeerPeriode ? {kwart: td.dataset.corrigeerPeriode} : {});
  });
  /* Speeltijdtabel: bij veel periodes (of een smal scherm) past niet alles naast
     elkaar. We schuiven de actieve periode dan in beeld, links van de vastgezette
     Totaal-kolom. Ook na het openklappen, want dan pas is de breedte bekend. */
  { const sp = v.querySelector('#speeltijdUitklap');
    if (sp){
      const naarNu = () => {
        const sc = sp.querySelector('.st-scroll'); if (!sc || !sc.clientWidth) return;
        const nu = sc.querySelector('th.nu'), tot = sc.querySelector('th.tot');
        if (!nu) return;
        const doel = nu.offsetLeft + nu.offsetWidth + (tot ? tot.offsetWidth : 0) - sc.clientWidth;
        if (doel > 0) sc.scrollLeft = doel;
      };
      sp.addEventListener('toggle', naarNu);
      naarNu();
    } }
...
function koppelSleep(v){
  ...
  // Eén gedelegeerde handler op het hele veld (i.p.v. per slot), zodat een tik
  // op de rand van een slot of op de 'M1'-ring altijd wordt opgevangen — ook
  // bij zelfgemaakte formaties. Klikken op een veldspeler-chip wordt door de
