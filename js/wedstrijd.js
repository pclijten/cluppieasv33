...
import { openInvoegSheet, bewaarWedstrijdAlsSjabloon, zetNaToepassenCallback } from './opstelling-sjabloon.js?v=20260924a';
...
export function openWedstrijd(wid){
  ...
  if (!S.teamId || !wid){
    ...
    S.wedstrijdId = null;
    if (S.teamId) import('./teams.js?v=20260924a').then(m => m.renderTeam?.());
    return;
  }
...
export function sluitWedstrijd(naarTab){
  ...
  bewaarPositie();
  import('./teams.js?v=20260924a').then(m => { m.renderTeam(); toon('team'); });
}
...
export function koppelStatsBlad(root){
  (root || document).querySelectorAll('[data-statsblad]').forEach(b => b.onclick = () => {
    ...
    import('./teams.js?v=20260924a').then(m => m.renderTeam?.());
  });
}
...
${confroHtml}
  ...
  if (teamEvalKnop) teamEvalKnop.onclick = () => {
    import('./teams.js?v=20260924a').then(m => m.modalTeamEvaluatie(S.wedstrijdId));
  };
