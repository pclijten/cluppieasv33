...
import { kwartGespeeld, effectieveLineup, analyseKwart, analyseWedstrijd, speeltijdReserve, disciplinaireTijd } from './analyse.js?v=20260902b';
import { ico } from './icons.js?v=20260825b';

import { telGebruik, telNav } from './tracker.js?v=20260902b';
import { openInvoegSheet, bewaarWedstrijdAlsSjabloon, zetNaToepassenCallback } from './opstelling-sjabloon.js?v=20260902b';
...
export function openWedstrijd(wid){
  ...
  if (!S.teamId || !wid){
    ...
    S.wedstrijdId = null;
    if (S.teamId) import('./teams.js?v=20260902b').then(m => m.renderTeam?.());
    return;
  }
...
export function sluitWedstrijd(naarTab){
  ...
  bewaarPositie();
  import('./teams.js?v=20260902b').then(m => { m.renderTeam(); toon('team'); });
}
...
export function koppelStatsBlad(root){
  (root || document).querySelectorAll('[data-statsblad]').forEach(b => b.onclick = () => {
    ...
    import('./teams.js?v=20260902b').then(m => m.renderTeam?.());
  });
}
...
${confroHtml}
  ...
  if (tbk) tbk.onclick = () => import('./tactiekbord.js?v=20260902b').then(m => m.openTactiekLijst(w));
  ...
  if (teamEvalKnop) teamEvalKnop.onclick = () => {
    import('./teams.js?v=20260902b').then(m => m.modalTeamEvaluatie(S.wedstrijdId));
  };
