...
import { laadPdfJs } from './pdf-viewer.js?v=20260819c';
...
async function veldDiagramBlob(page){
  ...
  // Verzamel ALLE geschilderde afbeeldingen op de pagina met hun naam, positie
  // en oppervlak, en kies daarna de GROOTSTE. Dat is altijd het veld-diagram.
  // (Voorheen pakten we de eerste afbeelding — maar sommige pagina's beginnen
  // met een klein logo/avatar-icoontje linksbovenaan; dat kwam dan als \"diagram\"
  // door i.p.v. het veld. De grootste-afbeelding-heuristiek voorkomt dat.)
  const kandidaten = [];
  let ctm = [1,0,0,1,0,0];
  const stack = [];
  ...
  if (!kandidaten.length){ console.warn('[training-ai] geen afbeelding op pagina gevonden'); return null; }
  // grootste geschilderde afbeelding = het veld-diagram
  kandidaten.sort((a, b) => b.opp - a.opp);
  const gekozen = kandidaten[0];
  const naam = gekozen.naam;
  const rect = gekozen.rect;
