...
async function zorgWedstrijdOpen(){
  ...
  let m;
  try { m = await import('./wedstrijd.js?v=20260823a'); }
...
async function verlaatWedstrijd(){
  ...
  try {
    const m = await import('./wedstrijd.js?v=20260823a');
...
export const ONBOARDING_STAPPEN = [
  ...
  /* ══════════════ 1. TEAM OPENEN ══════════════ */
  {
    ...
    voor:async () => {
      if (!document.querySelector('#view-teams.actief')){
        try { const t = await import('./teams.js?v=20260823a'); t.startTeams?.(); } catch(e){ console.warn('[ob] teams.js laadfout', e); }
  ...
  },
  {
    ...
    voor:async () => { S.teamTab = 'instellingen'; const m = await import('./teams.js?v=20260823a'); m.renderTeam?.();
