...
import { db, doc, getDoc, setDoc } from './firebase.js?v=20260922c';
import { S, $, $$, esc, meld, modAan, toon } from './state.js?v=20260922c';
...
async function zorgWedstrijdOpen(){
  ...
  let m;
  try { m = await import('./wedstrijd.js?v=20260925a'); }
...
async function verlaatWedstrijd(){
  ...
  try {
    const m = await import('./wedstrijd.js?v=20260925a');
...
export const ONBOARDING_STAPPEN = [
  ...
  /* ══════════════ 1. TEAM OPENEN ══════════════ */
  {
    ...
    voor:async () => {
      if (!document.querySelector('#view-teams.actief')){
        try { const t = await import('./teams.js?v=20260925a'); t.startTeams?.(); } catch(e){ console.warn('[ob] teams.js laadfout', e); }
  ...
  /* ══════════════ 11. TEAMINSTELLINGEN ══════════════ */
  {
    ...
    tekst:'Via de tegel <b>Instellingen</b> (onder Meer) regel je de teamnaam, de speelcategorie en je eigen weergavenaam.',
    opdracht:'Open Instellingen',
    voor:() => naarTab('meer'),
    doel:() => meerTegel('instellingen'),
    wacht:(klaar) => bijTabWissel('instellingen', klaar),
  },
  {
    ...
    voor:async () => { S.teamTab = 'instellingen'; const m = await import('./teams.js?v=20260925a'); m.renderTeam?.();
...
/* Als het uitgelichte doel (bv. een knop rechtsboven) onder de HUD-balk
